...
class HTTP_Request2 implements SplSubject
{
  ...
  public function __construct(
    ...
    $this->setHeader(
      'user-agent',
      'HTTP_Request2/2.3.0 ' .
        '(http://pear.php.net/package/http_request2) PHP/' .
        phpversion()
    );
  }
  ...
  public function addCookie($name, $value)
  {
    if (!empty($this->cookieJar)) {
      $this->cookieJar->store(['name' => $name, 'value' => $value], $this->url);
    } else {
      $cookie = $name . '=' . $value;
      ...
      $cookies = empty($this->headers['cookie'])
        ? ''
        : $this->headers['cookie'] . '; ';
  ...
  public function getBody()
  {
    if (
      ...
      if (
        ...
        0 === strpos($this->headers['content-type'], 'multipart/form-data')
      ) {
