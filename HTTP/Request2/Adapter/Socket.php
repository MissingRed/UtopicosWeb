...
class HTTP_Request2_Adapter_Socket extends HTTP_Request2_Adapter
{
  ...
  protected function connect()
  {
    ...
    $remote =
      (!$secure || $httpProxy || $socksProxy ? 'tcp://' : 'tls://') .
      $host .
      ':' .
      $port;
    $socketKey =
    ...
    if (
      ...
      if ($socksProxy) {
        ...
        $this->socket = new HTTP_Request2_SOCKS5(
          ...
          $options,
          $this->request->getConfig('proxy_user'),
          $this->request->getConfig('proxy_password')
        );
        // handle request timeouts ASAP
        $this->socket->setDeadline(
          $deadline,
          $this->request->getConfig('timeout')
        );
      ...
      $this->request->setLastEvent(
        'connect',
        empty($conninfo) ? $remote : $conninfo
      );
   ...
   * @return   boolean
   */
  protected function canKeepAlive(
    $requestKeepAlive,
    ...
    $lengthKnown =
      'chunked' == strtolower($response->getHeader('transfer-encoding')) ||
    ...
    $persistent =
      'keep-alive' == strtolower($response->getHeader('connection')) ||
  ...
  protected function handleRedirect(
    ...
    $redirectUrl = new Net_URL2($response->getHeader('location'), [
  ...
  protected function shouldUseServerDigestAuth(HTTP_Request2_Response $response)
  {
    ...
    if (
      !($challenge = $this->parseDigestChallenge(
    ...
    if ($port = $url->getPort()) {
      if (
        (0 == strcasecmp($scheme, 'http') && 80 != $port) ||
  ...
  protected function prepareHeaders()
  {
    ...
    if ($connect) {
      ...
      $query = $url->getQuery();
      $requestUrl .=
        (empty($path) ? '/' : $path) . (empty($query) ? '' : '?' . $query);
    }
    ...
    if (
      ...
      $headers['cookie'] =
        (empty($headers['cookie']) ? '' : $headers['cookie'] . '; ') . $cookies;
    }
  ...
  protected function readResponse()
  {
    ...
    do {
      try {
        ...
        if (
          HTTP_Request2_Exception::TIMEOUT === $e->getCode() &&
    ...
    if ($chunked || null === $length || 0 < intval($length)) {
      while (!$this->socket->eof() && (is_null($toRead) || 0 < $toRead)) {
        ...
        if (
          !in_array($response->getHeader('content-encoding'), [
            'identity',
            null,
          ])
        ) {
