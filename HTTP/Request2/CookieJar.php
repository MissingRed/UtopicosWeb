...
class HTTP_Request2_CookieJar implements Serializable
{
   ...
   */
  protected function checkAndUpdateFields(
    array $cookie,
  ...
  public function store(array $cookie, Net_URL2 $setter = null)
  {
    ...
    if (
      ...
      isset($this->cookies[$cookie['domain']][$cookie['path']][$cookie['name']])
    ) {
      unset(
   ...
   * @throws HTTP_Request2_LogicException
   */
  public function addCookiesFromResponse(
  ...
  public function domainMatch($requestHost, $cookieDomain)
  {
    ...
    if (
      (!$this->useList && substr_count($cookieDomain, '.') < 2) ||
